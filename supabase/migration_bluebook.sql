-- Magic Log (Alberta MVP) — extends public.users (app uses `users`, not `profiles`)
-- Fresh install: run this file. Already on bluebook_*? Run migration_rename_bluebook_to_magiclog.sql first.
...
alter table public.users add column if not exists magiclog_onboarding_complete boolean not null default false;

create table if not exists public.magiclog_work_orders (
...
create index if not exists magiclog_work_orders_user_id_idx on public.magiclog_work_orders(user_id);
create index if not exists magiclog_work_orders_created_at_idx on public.magiclog_work_orders(created_at desc);

create table if not exists public.hour_logs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.users(id) on delete cascade,
  work_order_id uuid references public.magiclog_work_orders(id) on delete set null,
...
alter table public.magiclog_work_orders enable row level security;
alter table public.hour_logs enable row level security;
alter table public.period_progress enable row level security;

create policy "magiclog_work_orders select own"
on public.magiclog_work_orders for select
using (auth.uid() = user_id);

create policy "magiclog_work_orders insert own"
on public.magiclog_work_orders for insert
with check (auth.uid() = user_id);

create policy "magiclog_work_orders update own"
on public.magiclog_work_orders for update
using (auth.uid() = user_id)
with check (auth.uid() = user_id);

create policy "magiclog_work_orders delete own"
on public.magiclog_work_orders for delete
using (auth.uid() = user_id);
...
create table if not exists public.magiclog_reminder_logs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.users(id) on delete cascade,
...
create index if not exists magiclog_reminder_logs_user_type_idx
  on public.magiclog_reminder_logs(user_id, reminder_type, sent_at desc);

alter table public.magiclog_reminder_logs enable row level security;

create policy "magiclog_reminder_logs service only"
on public.magiclog_reminder_logs for all
using (false)
with check (false);

...
/** Split steps across two A4 pages (first half / second half). */
export function splitStepsForWorkOrderPages<T>(steps: T[]): [T[], T[]] {
  if (steps.length <= 4) return [steps, []];
  const mid = Math.ceil(steps.length / 2);
  return [steps.slice(0, mid), steps.slice(mid)];
}

...
import { syncPeriodProgress } from "@/lib/bluebook/computeProgress";
import { fetchBluebookProfile } from "@/lib/bluebook/profile";
import { sendBluebookReminderEmail } from "@/lib/bluebook/sendEmail";
    ...
    if (!latestOrder?.created_at) {
      if (daysBetween(accountStart, now) >= 3) {
        const status = await sendReminder(
          ...
          `Hi,\n\nYou haven't logged a work order in My Bluebook yet. Add today's task so your period hours and competences stay up to date.\n\n— LinkSyllabus My Bluebook`
        );
      ...
      if (daysBetween(last, now) >= 3) {
        const status = await sendReminder(
          ...
          "no_work_order_3d",
          "Don't forget to log your work today",
          `Hi,\n\nIt's been a few days since your last work order in My Bluebook. Log what you worked on today to keep your blue book current.\n\n— LinkSyllabus My Bluebook`
        );
        results.push({ userId, type: "no_work_order_3d", status });
        if (status === "sent") sent += 1;
      }
    ...
    if (end && !progress.period_complete) {
      ...
      if (daysUntilEnd >= 0 && daysUntilEnd <= 30) {
        const status = await sendReminder(
          ...
          `Hi,\n\nYou're within 30 days of completing Period ${period} on your apprenticeship timeline. Check your progress in My Bluebook and finish any remaining competences and hours.\n\n— LinkSyllabus My Bluebook`
        );

...
export function DashboardClient() {
  ...
  useEffect(() => {
    fetch("/api/bluebook/dashboard")
      .then((r) => r.json())
      .then((j) => {
        if (j.profile && !j.profile.bluebook_onboarding_complete) {
          router.replace("/bluebook/onboarding");
          return;
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="flex flex-wrap gap-3">
        <Link href="/bluebook/new" className="btn-primary inline-flex">
          + New work order
        </Link>
        <Link href="/bluebook/export" className="btn-ghost inline-flex">
          Export &amp; print
        </Link>
      </section>

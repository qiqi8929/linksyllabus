...
export function OnboardingWizard() {
  ...
  async function startCheckout() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/bluebook/checkout", { method: "POST" });
      const j = await res.json();
  ...
  async function finishWithoutCheckout() {
    ...
    try {
      ...
      router.replace("/bluebook/dashboard");
      router.refresh();
    } catch (e: unknown) {
  ...
  return (
    <div className="card mx-auto max-w-lg p-6">
      <h1 className="text-xl font-semibold">Set up My Bluebook</h1>
